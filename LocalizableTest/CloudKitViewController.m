...
- (void)initViews {
    ...
    self.textView.frame = CGRectMake(0, 0, 200, 100);
    self.textView.center = CGPointMake(self.view.center.x, self.view.center.y-150);
    ...
    self.textField.center = CGPointMake(self.view.center.x, self.view.center.y-40);
...
- (void)query {
    ...
    [self.db performQuery:query inZoneWithID:nil completionHandler:^(NSArray<CKRecord *> * _Nullable results, NSError * _Nullable error) {
...
- (void)initData {
    NSDate *birth = [self dateFromString:@"1992-10-29 00:00:00"];
    NSString *uuidString = [[[UIDevice currentDevice] identifierForVendor] UUIDString];
    self.db = [[CKContainer defaultContainer] privateCloudDatabase];
    if (!self.record) {
        CKRecordID *recordId = [[CKRecordID alloc] initWithRecordName:uuidString];
        self.record = [[CKRecord alloc] initWithRecordType:@"Establishment" recordID:recordId];
    }
    [self.record setObject:self.textField.text forKey:@"name"];
    [self.record setObject:birth forKey:@"birth"];
    [self.record setObject:self.currentLocation forKey:@"location"];
    [self.record setObject:@(24) forKey:@"age"];
    [self.db saveRecord:self.record completionHandler:^(CKRecord * _Nullable record, NSError * _Nullable error) {
        if (error) {
            NSLog(@"%@", error);
        } else {
            NSLog(@"success");
